...
userSchema.statics.register = async function(email, password, sex, activity, height, weight, goal, age){
    if(!email || !password || !sex || !activity || !height || !weight || !goal || !age){
    ...
    const userData = { email, password: hash, sex, activity, height, weight, goal, age };
...
userSchema.statics.updateUser = async function (id, updates) {
    const allowedUpdates = ["email", "password", "sex", "activity", "height", "weight", "goal", "age"];
    ...
    if (keys.some((key) => ["sex", "activity", "height", "weight", "goal", "age"].includes(key))) {
